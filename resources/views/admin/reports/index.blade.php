        ...
        <form method="GET" action="{{ route('admin.reports.index') }}" class="flex flex-col gap-4">
            ...
            <div class="w-full sm:w-64">
                ...
                <input
                    type="date"
                    name="start_date"
                    value="{{ request('start_date', $dailyStats['start_date'] ?? '') }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg"
                >
            </div>
            ...
            <div class="w-full sm:w-64">
                ...
                <input
                    type="date"
                    name="end_date"
                    value="{{ request('end_date', $dailyStats['end_date'] ?? '') }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg"
                >
            </div>
            ...
            <div class="flex flex-wrap gap-2 items-end">
                ...
                <a
                    href="{{ route('admin.reports.index', ['date' => now()->format('Y-m-d')]) }}"
                    class="inline-flex items-center bg-white hover:bg-gray-50 text-gray-800 px-6 py-2 rounded-lg border border-gray-300"
                >
                    Today
                </a>
                <a
                ...
                <a
                    href="{{ route('admin.reports.daily-print', request()->only(['date'])) }}"
                    target="_blank"
                    rel="noopener"
                ...
                <a
                    href="{{ route('admin.reports.export.daily', request()->only(['date','start_date','end_date'])) }}"
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white rounded-lg shadow p-4">
                ...
                <p class="text-lg font-semibold text-gray-800 mt-1">
                    ...
                    @elseif(!empty($dailyStats['start_date']) || !empty($dailyStats['end_date']))
                        {{ $dailyStats['start_date'] ?? '' }} to {{ $dailyStats['end_date'] ?? '' }}
                    @else
                        All time
                    @endif
                </p>
