...
@section('content')
@php
    $myScheduleIds = $enrollment->schedules->where('faculty_id', auth()->id())->pluck('id')->values()->all();
@endphp
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-8">
        ...
        <p class="text-gray-600 mt-2 text-sm">Update <span class="font-semibold">your</span> class schedules for this student. Schedules from other instructors are kept unless an administrator changes them.</p>
    </div>

    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('faculty.enrollments.update', $enrollment->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select id="faculty-enrollment-edit-section-id" name="section_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('section_id') border-red-500 @enderror">
                ...
                <div class="enrollment-schedule-copy">
                    ...
                    <p class="enrollment-schedule-desc">Check the classes <span class="font-bold">you</span> teach for this student. Clearing all means your courses use the default (all classes in section) only if no other schedules are stored—contact an admin if the full list looks wrong.</p>
                    <div class="schedule-picker space-y-2 rounded-xl p-4 max-h-64 overflow-y-auto mt-1">
                        <div id="faculty-enrollment-edit-schedule-options" class="space-y-2"></div>
                        <p id="faculty-enrollment-edit-empty-selected" class="text-sm schedule-picker-hint text-slate-600 hidden">You have no schedules for this section.</p>
                        <p id="faculty-enrollment-edit-empty-unselected" class="text-sm schedule-picker-hint text-slate-600">Select a section to see your schedules.</p>
                    </div>
                    @error('schedule_ids')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
...
document.addEventListener('DOMContentLoaded', function () {
    const sectionSelect = document.getElementById('faculty-enrollment-edit-section-id');
    const optionsWrap = document.getElementById('faculty-enrollment-edit-schedule-options');
    const emptySelected = document.getElementById('faculty-enrollment-edit-empty-selected');
    const emptyUnselected = document.getElementById('faculty-enrollment-edit-empty-unselected');

    const schedulesBySection = @json($schedulesBySection);
    const selectedOnLoad = new Set(@json(array_map('intval', (array) old('schedule_ids', $myScheduleIds))));
