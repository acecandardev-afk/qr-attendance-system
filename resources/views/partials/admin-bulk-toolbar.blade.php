...
<div class="flex flex-wrap items-center gap-3 px-6 py-3 bg-slate-50 border-b border-slate-200">
    ...
    >
        {{ $archive ? 'Archive selected' : 'Delete selected' }}
    </button>
...
<template x-teleport="body">
    ...
    >
        <div
            ...
            @click.stop
        >
            <h3 class="text-lg font-semibold text-slate-900">{{ $archive ? 'Archive' : 'Delete' }} selected <span x-text="itemLabel"></span>?</h3>
            <p class="mt-2 text-sm text-slate-600">
                @if($archive)
                    You are about to archive <strong x-text="selectedCount"></strong> record(s). You can restore them later from the database if needed.
                @else
